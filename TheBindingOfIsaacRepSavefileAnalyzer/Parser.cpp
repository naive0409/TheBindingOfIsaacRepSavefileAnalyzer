#pragma once
#include "Parser.h"
#include <map>

u8 Parser::readU8()
{
    u8 res = 0;
    /*	for (int i = 0; i < 1; i++)
        {*/
    // res += (*it++) << 8 * i;
    res += *((this->iter)++);
    //}
    return res;
}

u16 Parser::readU16()
{
    u16 res = 0;
    for (int i = 0; i < 2; i++)
    {
        res += *((this->iter)++) << 8 * i;
    }
    return res;
}

u32 Parser::readU32()
{
    u32 res = 0;
    for (int i = 0; i < 4; i++)
    {
		res += *((this->iter)++) << 8 * i;
    }
    return res;
}

u32 Parser::getU32(vector<u8>::iterator iter)
{
	u32 res = 0;
	for (int i = 0; i < 4; i++)
	{
		vector<u8>::iterator temp_it = (this->iter) + i;
		res += *(temp_it) << 8 * i;
	}
	return res;
}

void Parser::parse(vector<u8>& unparsedData) {
	//int savedata; //JSON
	string preleading = "";
	u32 checksum = 0;
	vector<bool> achis;
	map<string, u32> counter;
	vector<u32> conter_data;

	//std::vector<u8> v = { 73,83,65,65,67,78,71,83,65,86,69,48,57,82,32,32,93,150,51,254 };

	this->iter = unparsedData.begin();

	for (int i = 0; i < 16; i++)
	{
		preleading += *((this->iter)++);
		//cout << *(iter + i) << ',';
	}

	cout << preleading << endl;

	if (preleading != "ISAACNGSAVE09R  ") {
		cout << "您的存档格式不受支持，只支持忏悔(Rep)的存档";
		return;
	}

	checksum = readU32();
	cout << checksum << endl;


	int block_count = 0;
	while (block_count < 11) {
		block_count++;
		u32 block_type = readU32();
		u32 block_size = readU32();
		u32 extra_number = readU32();

		switch (block_type) {
		case 1:
		{
			u32 achi_count = extra_number;
			//vector<bool> achis;
			for (u32 i = 0; i < block_size; i++)
			{
				if (i < achi_count) achis.push_back(readU8() > 0);
			}
			break;
		}
		case 2:
		{
			iter++;
			preview(150);
			//cout 
			//	<< achis.size() << endl
			//	<< iter - unparsedData.begin()<<endl;
			//cout << "test" << endl;
			counter["momkills"] = getU32(iter + 4);
			counter["death"] = getU32(iter + 0x28);
			counter["beststreak"] = getU32(iter + 0x5c);
			cout 
				<< counter["momkills"] << endl
				<< counter["death"] << endl
				<< counter["beststreak"] << endl;
			break;
		}
		case 4:
		case 7:
		case 8:
		case 3:
		case 9:
		case 5:
		case 6:
		case 10:
		case 11:
		default:
			break;
		};
		
		//for (vector<bool>::iterator it = achis.begin(); it < achis.end(); it++)
		//{
		//	cout << boolalpha << *it << ',';
		//}


	}

}

